			...
			if(Object.getOwnPropertyNames(data).length === 0){
				window.location.replace("/loginpage");
			      ...
			      type: "GET",
			        ...
			        result.forEach(function(item){
			           $(".cartItems").append("<img class='prodimg rounded-circle ' width='50rem' style='height:50px'src='"+item.Product.product_image+"'>"+item.Product.product_name+"<br>$"+item.Product.price+"<br>Qty: "+item.cart_quantity+"<br>");
			        });
			    });
			}
		});	
